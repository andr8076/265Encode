...
WORK=${ENCODE265_QUALITY_RUNTIME_WORK:-$DEFAULT_WORK}
OUT=${ENCODE265_QUALITY_RUNTIME_OUT:-$DEFAULT_OUT}
JOBS=${ENCODE265_QUALITY_RUNTIME_JOBS:-}
if [[ -z $JOBS ]]; then
    if command -v nproc >/dev/null 2>&1; then JOBS=$(nproc)
    else JOBS=$(sysctl -n hw.logicalcpu 2>/dev/null || printf 4); fi
fi
[[ $JOBS =~ ^[1-9][0-9]*$ ]] || { printf 'ENCODE265_QUALITY_RUNTIME_JOBS must be a positive integer.\n' >&2; exit 2; }
...
validate_build_root() {
    ...
    marker="$path/.265encode-quality-runtime-build-root"
    case $path in
        /|"$BUILD_HOME"|"$ROOT")

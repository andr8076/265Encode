#!/usr/bin/env python3
"""Protocol-v2 semantic planning for 265Encode."""
...
from hevcplan_quality import predict
...
def calculate_plan_id(plan: dict[str, Any]) -> str:
    return "hevcp_"+hashlib.sha256(canonical(plan_payload(plan))).hexdigest()
...
def verify_plan(plan: dict[str, Any], script: Path) -> None:
    ...
    if recipe!=expected:raise PlanError("Plan recipe is not the current 265Encode policy recipe.")
    ...
    root=script.parent.resolve();current={"implementation":implementation_fingerprint(root),"runtime":runtime_fingerprint(encoder,expected),"source":source_fingerprint(Path(requirements["input"])),"requirements":{"algorithm":"sha256","value":digest(requirements)}};saved=plan.get("fingerprints")
...
def negotiate(text: str) -> dict[str, Any]:
    ...
    common=sorted(offered.intersection(SUPPORTED_PROTOCOLS));return {"schema":"encode265.negotiation","supported_protocol_versions":list(SUPPORTED_PROTOCOLS),"selected_protocol_version":common[-1] if common else None,"compatible":bool(common)}


def parser()->argparse.ArgumentParser:
    result=argparse.ArgumentParser(prog="HEVCPlan.py");sub=result.add_subparsers(dest="operation",required=True);n=sub.add_parser("negotiate");n.add_argument("versions");e=sub.add_parser("evaluate",aliases=["plan"]);e.add_argument("requirements",type=Path);e.add_argument("plan_json",type=Path);e.add_argument("encoder_script",type=Path);x=sub.add_parser("execute");x.add_argument("plan_json",type=Path);x.add_argument("result_json",type=Path);x.add_argument("encoder_script",type=Path);return result
...
def main(argv:list[str]|None=None)->int:
    ...
    try:
        ...
        if args.operation in {"evaluate","plan"}:
            plan=evaluate(args.requirements,args.plan_json,args.encoder_script.resolve());print(json.dumps({"schema":"encode265.plan-reference","protocol_version":2,"plan_id":plan["plan_id"],"plan":str(args.plan_json.resolve()),"prediction":plan["prediction"]},sort_keys=True));return 0
        ...
        error={"schema":"encode265.error","protocol_version":2,"status":"failed","error":str(exc)}

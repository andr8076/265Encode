...
from hevcplan_contract import PlanError, command_output
...
def execute_direct(requirements: dict[str,Any], recipe: dict[str,Any], source: dict[str,Any])->dict[str,Any]:
    output=Path(requirements["output"]);partial=output.with_name(f".{output.name}.encode265-partial-{os.getpid()}.mkv")
    try:
        ...
        if process.returncode!=0:raise PlanError(process.stderr.strip() or "HEVC execution failed.")
